...
class GraphNetBlock(nn.Module):
    ...
    def _update_node_features(self, node_features, edge_sets):
        ...
        for edge_set in edge_sets:
            ...
            features_dim = edge_set_features.shape[-1]
            seg_sum = torch.zeros((num_nodes, features_dim), dtype = edge_set_features.dtype).cuda()
            seg_sum.scatter_add_(dim = 0, index = edge_set_receivers.unsqueeze(1).repeat(1, features_dim), src =edge_set_features )
